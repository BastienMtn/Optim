...
public class Descente extends Observable implements Runnable {
	...
	private double seuil = 0.00001; // si le gradient a une longueur inferieure a seuil, la methode de descente
									// s'arrete
	...
	private boolean atteintMinimum = true; // passe a faux si la methode de descente montre que le problÃ¨me n'est pas
											// borne
	private int delai = 2000; /*
								 * sert a ralentir la descente pour l'affichage graphique ;
	...
	/**
	 * Un double est considere comme valant 0 si sa valeur absolue est inferieure a
	 * epsilon
	 * 
	 * @param v le double considere
	 * @return true si le nombre est considere comme nul, false dans le cas
	 *         contraire
	 */
	...
	/**
	 * @param pb le probleme considere ; le probleme consiste toujours a  chercher
	 *           le minimum
	 *           d'une fonction convexe de deux variables sur un domaine du plan
	 *           limite par des droites.
	 */
	 ...
	 * Fait trois actions :
	 * <br>
	 * - notifie les observateurs, ici l'interface graphique, afin que celle-ci
	 * actualise l'affichage
	 * <br>
	 * - suspend le thread en cours d'execution pendant un nombre minimum de
	 * millisecondes egal a la valeur de delai
	 * <br>
	 * - si la methode est suspendue, attend jusqu'a recevoir une notification quand
	 * la methode n'est plus
	 * suspendue.
	 */
	...
	/**
	 * La methode de descente est lancee dans un thread faisant tourner cette
	 * methode run.
	 */
	public void run() {
		if (P != null)
	 ...
	 * Si la descente n'est pas finie, met la prochaine direction dans l'attribut
	 * this.direction
	 * (c'est obligatoire d'utiliser cet attribut).
	 * <br>
	 * ATTENTION : si la methode de descente est finie, la methode doit passer
	 * this.finie a true.
	 * 
	 * @see Couple#norme()
	...
	private void directionASuivreSiInterieur() {
		Couple grad = pb.gradientf(this.P);
		if (grad.norme() > seuil) {
			this.direction = grad.mult(-1 / grad.norme());
		} else {
			this.finie = true;
		}
	}
	 ...
	 * (c'est obligatoire d'utiliser cet attribut).
	 * <br>
	 * Selon la direction du gradient de f et si la descente n'est pas terminée, la
	 * prochaine direction
	 * peut etre vers l'intérieur strict du domaine ou le long du bord dans un sens
	 * ou dans l'autre.
	 * <br>
	 * Attention : si la methode de descente est finie, la methode doit passer
	 * this.finie a true.
	...
	private void directionASuivreSiBord(Contrainte c) {
		Couple grad = pb.gradientf(this.P);
		if (c.getGradient().produitScalaire(grad) >= 0) {
			this.direction = grad.mult(-1 / grad.norme());
		} else {
			Couple ubord = c.getVecteurUnitaireBord();
			if (grad.estPerpendiculaire(ubord)) {
				this.finie = true;
			} else if (grad.produitScalaire(ubord) < 0) {
				this.direction = ubord;
			} else {
				this.direction = ubord.mult(-1);
			}
		}
	}
	 ...
	 * this.finie a true.
	 * <br>
	 * Voir le sujet sur http://www.infres.enst.fr/~hudry/optim/descente/TPDescente/
	 ...
	 * @see Contrainte#getBordUnitaire()
	 * @see Pb#gradientf(Couple)
	 * @param coin un tableau a deux cases pour les deux contraintes saturees par le
	 *             point courant P
	 */
	private void directionASuivreSiCoin(Contrainte[] coin) {
		...
		if (grad.produitScalaire(u1) < 0 && grad.produitScalaire(u2) < 0) {
			this.direction = grad.mult(-1);
		} else if (grad.produitScalaire(n1) > 0 || grad.produitScalaire(n2) > 0) {
			this.finie = true;
		} else {
			if (grad.produitScalaire(u1) <= grad.produitScalaire(u2)) {
				this.direction = u1;
			} else {
				this.direction = u2;
			}
	 ...
	 * On rappelle que, par hypothese, la fonction f est convexe, ce qui entraine
	 * que phi' est croissante.
	 * On pourra d'abord tester t = 1 puis, si necessaire, doubler la valeur de t
	 * successivement.
	 ...
	 * @return si on ne trouve pas de tel point avec t < Double.MAX_VALUE/2, la
	 *         methode retourne -1 ;
	 *         <br>
	 *         sinon la methode retourne une valeur de t avec phi'(t) > 0.
	 */
	 ...
	 * <br>
	 * On peut utiliser la methode statique estNul de cette classe pour tester si
	 * une valeur
	...
	/**
	 * METHODE A IMPLEMENTER
	 * Verifie qu'il s'agit bien d'un minimum en utilisant la condition de Karush,
	 * Kuhn et Tucker
	 * 
	 ...
	 *         sinon
	 *         <br>
	 *         - le couple des multiplicateurs de Lagrange si P est sur un coin
	 *         <br>
	 *         - le couple forme par le multiplicateur de Lagrande et 0 si P est sur
	 *         un bord
	 *         <br>
	 *         - (0, 0) si P est a l'interieur
	 */
	public Couple KarushKuhnTucker(Couple P) {
		Couple lambda = null;
		if(domaine.estCoin(this.P)!=null){

		}else if(domaine.estSurBord(this.P)!=null){
	 ...
	 * <br>
	 * P est a  l'interieur du domaine,
	 * <br>
	 * P est sur un bord,
	 * <br>
	 * P est sur un coin
	 * <br>
	 * Si la descente est finie, la methode passe this.finie a true.
	 */
	 ...
	 * 
	 * @return
	 *         this.P est le point courant (qui peut etre a l'interieur de domaine,
	 *         sur un
	 *         bord du domaine ou sur un coin) et this.direction est la direction a
	 *         suivre ; la methode retourne
	 *         <br>
	 *         - soit le point courant suivant (qui peut etre a l'interieur du
	 *         domaine, sur un bord, sur un coin)
	 *         <br>
	 *         - soit null si elle a mis en evidence que la fonction f n'atteint pas
	 *         de minimum
	...
	public Couple pointSuivantDescente() {
		 ...
		 * cette demi-droite peut partir de l'interieur du domaine, ou d'un bord, ou
		 * d'un coin
		 * et peut longer un bord du domaine.
		 * Si la demi-droite est toute entiere dans le domaine, ce qui n'est possible
		 * que
		 * si le domaine n'est pas borne,l'instruction retourne -1.
		 * Sinon, elle retourne une valeur de t > 0 pour laquelle this.P + t *
		 * this.direction appartient
		 * a un bord du domaine.
		 */
		t1 = domaine.intersection(this.P, this.direction);
		if (t1 == 0)
			return this.P;
		// Si la demi-droite rencontre un bord du domaine et si la derivee de t ->
		// this.P + t * this.direction
		...
		// Si la demi-droite est toute entiere dans le domaine, on cherche un point ou
		// la derivee de
	 ...
	 * 
	 * @param suspendre si le parametre vaut true, la methode est suspendue, elle
	 *                  est reprise
	 ...
	 * 
	 * @param seuil si la norme du gradient est inferieur a  seuil, la methode de
	 *              descente s'arrete
	 */
	...
	/**
	 * @return temps d'attente entre deux pas de la methode de descente, en
	 *         millisecondes
	 */
	public int getDelai() {
		return delai;
	 ...
	 * 
	 * @param delai temps d'attente entre deux pas de la methode de descente, en
	 *              millisecondes
	 */
